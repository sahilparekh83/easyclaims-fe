...
import { adminListPartners, adminListMembers, adminListPolicies, adminListPlans } from "@/imports/core/api";
import StatusBadge from "@/components/ui/StatusBadge";
import { Users, FileText, CreditCard, ShieldCheck, Sparkles } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const { data: partnersData, isLoading: pL } = useQuery({ queryKey: ["admin","partners","total"], queryFn: () => adminListPartners(LIST_PARAMS) });
  const { data: membersData,  isLoading: mL } = useQuery({ queryKey: ["admin","members","total"],  queryFn: () => adminListMembers(LIST_PARAMS) });
  const { data: policiesData, isLoading: poL } = useQuery({ queryKey: ["admin","policies","total"], queryFn: () => adminListPolicies(LIST_PARAMS) });
  const { data: plansData,    isLoading: plL } = useQuery({ queryKey: ["admin","plans","total"],   queryFn: () => adminListPlans({}) });
  const { data: recentMembersData } = useQuery({ queryKey: ["admin","members","recent"], queryFn: () => adminListMembers(RECENT_PARAMS) });

  const totalMembers  = membersData?.data?.total ?? 0;
  const totalPolicies = policiesData?.data?.total ?? 0;
  const totalPartners = partnersData?.data?.total ?? 0;
  const totalPlans    = Array.isArray(plansData?.data) ? plansData.data.length : (plansData?.data?.total ?? 0);
  ...
  const KPIS = [
    ...
    { label: "Active partners", value: totalPartners, loading: pL,  bg: "#f0fdf4", color: "#16a34a", icon: <ShieldCheck size={18} /> },
        ...
        <SectionCard>
          <CardHeader>
            <div>
              ...
              <CardSub>New &amp; renewed this week</CardSub>
            </div>
            ...
            <tbody>
              {recentMembers.length === 0 ? (
                ...
                >
                  ...
                  <TdSm style={{ color: "#374151" }}>{m.enrollments?.[0]?.plan_name || "—"}</TdSm>
                  <TdSm style={{ color: "#374151" }}>{m.enrollments?.[0]?.partner_name || "—"}</TdSm>
